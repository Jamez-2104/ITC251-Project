...
**`scanf` can't handle spaces in input.**
Initially `scanf` was used to read the folder path and pattern from the user. `scanf` stops reading at the first space, so a path with spaces would break the search. This was fixed by switching to `fgets()`, which reads the entire line including spaces. The trailing newline that `fgets()` leaves is then removed with `strcspn()` so it doesn't interfere with the folder path or pattern.
